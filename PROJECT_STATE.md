...
| WhatsApp (Evolution API/Baileys): error 463 "reach-out time-lock" al responder a números nuevos | **🟡 EN VALIDACIÓN — Gates 1, 2 y 3 completos (transporte, estabilidad de sesión, y cobertura del 463 con 2 contactos fríos distintos, ambos con entrega real confirmada). Ver `docs/ops/whatsapp_evolution_maintenance.md` para el estado exacto por gate y el plan de la próxima sesión (Gate 4: integración real con backend+IA — el pendiente real ahora; Gate 5: artefacto reproducible; Gate 6: promoción formal). Detalle de toda la investigación en `docs/ops/whatsapp_463_technical_report.md`. Sigue siendo la prioridad de la próxima sesión — el backend real de la app todavía no está conectado a la build validada.** |
