...
| WhatsApp (Evolution API/Baileys): error 463 "reach-out time-lock" al responder a números nuevos | **🟢 VALIDADO A NIVEL FUNCIONAL, PENDIENTE PROMOCIÓN FORMAL — Gates 1-5 completos: transporte, estabilidad de sesión, cobertura del 463 (2 contactos fríos distintos), integración real completa (sin intervención manual, entrega confirmada), y artefacto reproducible vendorizado en `docker/evolution/evolution-api-fix-2608/` (Image ID idéntico confirmado en rebuild). Solo falta Gate 6 (promoción formal a `docker-compose`/producción real) — en curso. Pendiente no técnico: notificación de uso de Evolution API que exige su licencia, todavía no implementada. Ver `docs/ops/whatsapp_evolution_maintenance.md` para el detalle. Detalle de toda la investigación en `docs/ops/whatsapp_463_technical_report.md`.** |
