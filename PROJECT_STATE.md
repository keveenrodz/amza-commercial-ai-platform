...
| WhatsApp (Evolution API/Baileys): error 463 "reach-out time-lock" al responder a números nuevos | **🟢 VALIDADO A NIVEL FUNCIONAL, PENDIENTE PROMOCIÓN FORMAL — Gates 1-4 completos: transporte, estabilidad de sesión, cobertura del 463 (2 contactos fríos distintos), e integración real completa (WhatsApp → webhook → backend → IA → Evolution → WhatsApp, sin intervención manual, entrega confirmada). Ver `docs/ops/whatsapp_evolution_maintenance.md` para el detalle y el plan de Gates 5-6 (artefacto reproducible + promoción formal a `docker-compose`/producción) — eso es lo único que falta. Detalle de toda la investigación en `docs/ops/whatsapp_463_technical_report.md`.** |
